...
def main() -> None:
    ...
    parser.add_argument("--no-plots", action="store_true", help="Skip plots (faster for smoke tests).")
    parser.add_argument("--sigma-fraction", type=float, default=0.18, help="CMA-ES step size (fraction of bounds).")
    parser.add_argument("--popsize", type=int, default=6, help="CMA-ES population size.")
    args = parser.parse_args()

    definition = default_bboplace_definition(benchmark=args.benchmark, n_macro=args.n_macro, bench_seed=args.seed)

    results_root = Path(__file__).resolve().parents[1] / "runs" / "demo" / f"bboplace_{args.benchmark}_nmacro{args.n_macro}"
