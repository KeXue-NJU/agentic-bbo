...
The RF checkpoint (`.joblib`) is **not** shipped in git: download the released file and install it as described in ``bbo/tasks/surrogate/assets/README.md`` (shared download link). This repository **implements** the evaluation path (joblib load + knob JSON decoding) only.
